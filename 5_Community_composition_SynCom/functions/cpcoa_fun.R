...
plot_cap <- function(p, d, col_var, pch_var, col_comp, pch_comp, shapes, 
                     ...
                     cap_var, perm_anova, cex.main = 0.8, cex = 0.8) {
	...
	for (i in 1:length(pch_var)) {
		...
		main <- paste(constraint, ": [", 
                      format(var_tbl["constrained", "proportion"] * 100, 
                             digits=2), 
                      "% of variance; P < ", 
                      format(perm_anova[1, 5], digits = 2),
...
plot_pcoa_spp <- function(p, centroids = NULL, pch, file_name, svg = F, 
                          pcoa_var, abundances = NULL, otu_subset = NULL, 
                          taxonomy = NULL, cex.main = 0.8, cex = 0.8) {

	col <- rep("black", dim(p)[1])

	if(!is.null(taxonomy)) {
		col[taxonomy[rownames(p), ]$Phylum == "Proteobacteria"] <- "green4"
		col[taxonomy[rownames(p), ]$Phylum == "Bacteroidetes"] <- "blue1"
		col[taxonomy[rownames(p), ]$Phylum == "Actinobacteria"] <- "red1"
	}
...
plot_biplots <- function(p, centroids, pch, col, file_name, svg = F, constraint, 
                         ci, var_tbl, cap_var, perm_anova, abundances = NULL, 
	...
	main <- paste(constraint, ": [", 
                  format(var_tbl["constrained", "proportion"] * 100, digits = 2),
      			  "% of variance; P < ", format(perm_anova[1, 5], digits = 2), 
			      "; 95% CI = ", format(ci[1] * 100, digits = 2), 
			      "%, ", format(ci[2] * 100, digits = 2), "%]", sep="")	
	if (svg) svg(file = file_name) else pdf(file = file_name)

	if (!is.null(taxonomy)) {
		colors[taxonomy[rownames(p),]$Phylum == "Proteobacteria"] <- "seagreen4"
		colors[taxonomy[rownames(p),]$Phylum == "Bacteroidetes"] <- "steelblue2"
		colors[taxonomy[rownames(p),]$Phylum == "Actinobacteria"] <- "red"
	}
