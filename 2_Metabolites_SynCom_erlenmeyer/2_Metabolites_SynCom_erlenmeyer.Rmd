...
```{r, message=FALSE, echo=FALSE, error=FALSE, warning = FALSE, fig.dim = c(7,4)}
Treatment.labs <- c("DMSO", "MBOA low", "MBOA high")
names(Treatment.labs) <- c("DMSO", "MBOA_low", "MBOA_high")

met_erlenemyer <- met %>% 
  mutate(Strain = gsub("S", "SCS", Strain)) %>% 
  mutate(Strain = gsub("R", "SCR", Strain)) %>% 
  filter(!Treatment %in% "AMPO") %>% 
  filter(Compound %in% c("MBOA", "AAMPO", "AMPO")) %>% 
  ggplot(aes(x = Strain, y = uMol, group = Compound)) +
  geom_bar(aes(fill = Compound), stat="identity", position="stack", size = 2, show.legend = TRUE) +
...
# ggsave(plot = met_erlenemyer, filename = "met_erlenemyer.pdf",width = 13.2, height = 8, dpi = 300, scale = 1, units = "cm")
# 
# ggsave(plot = met_erlenemyer, filename = "met_erlenemyer.svg", width = 13.2, height = 8, dpi = 300, scale = 1, units = "cm")
```
...
# ggsave(plot = met_area_erlenemyer, filename = "met_area_erlenemyer.pdf",width = 13.2, height = 8, dpi = 300, scale = 1, units = "cm")
# 
# ggsave(plot = met_area_erlenemyer, filename = "met_area_erlenemyer.svg", width = 13.2, height = 8, dpi = 300, scale = 1, units = "cm")
```
